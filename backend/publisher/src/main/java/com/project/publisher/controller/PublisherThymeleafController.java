...
public class PublisherThymeleafController {
    ...
    public String home(Model model, @RequestParam(required = false, defaultValue = "0") long timestamp) {
        ...
        model.addAttribute("topics", publisherService.getTopics());
        model.addAttribute("timestamp", publisherService.getLogicalClock());
        return "publisher";
    }

    @GetMapping("/create-topic")
    public String createTopicForm(Model model, @RequestParam(required = false, defaultValue = "0") long timestamp) {
        publisherService.updateClock(timestamp);
        model.addAttribute("timestamp", publisherService.getLogicalClock());
        return "create-topic";
    }
    ...
    public String createTopic(@RequestParam String topic, @RequestParam(required = false, defaultValue = "0") long timestamp) {
        publisherService.updateClock(timestamp);
